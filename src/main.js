document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!utils) {
        console.error('[renderer] appUtils not found.');
        return;
    }
    ...
    invoke('get_position_lock_state').then((locked) => {
    ...
    pinBtn.addEventListener('click', () => {
        ...
        invoke('toggle_position_lock', { shouldLock: newState }).then((locked) => {
            isPositionLocked = locked;
            updateLockState(locked);
            try {
                localStorage.setItem('isPositionLocked', JSON.stringify(locked));
            } catch (err) {
